...
municipality_ids = [row[0] for row in db.execute(
    "SELECT id FROM classes WHERE title = 'municipality'").fetchall()]
rooms_ids = [row[0] for row in db.execute(
    "SELECT id FROM classes WHERE title = 'rooms'").fetchall()]
condition_ids = [row[0] for row in db.execute(
    "SELECT id FROM classes WHERE title = 'condition'").fetchall()]
property_type_ids = [row[0] for row in db.execute(
    "SELECT id FROM classes WHERE title = 'property_type'").fetchall()]
